...
class PrintingScreen(tk.Frame):
    ...
    def on_show(self):
        ...
        existing_job_id = self.manager.state.get("current_job_id") if self.manager else None
        if not form_data:
        ...
        self._worker = threading.Thread(
            target=self._run,
            args=(form_data, existing_job_id),
            daemon=True,
        )
        self._worker.start()

    def _run(self, form_data: dict, existing_job_id: str | None = None):
        def on_status(code: str):
            self.after(0, lambda: self._set_status(code))

        result = run_print_job(form_data, existing_job_id=existing_job_id, on_status=on_status, do_print=True, do_db_insert=True)
        self._last_result = result
        if self.manager:
            self.manager.state["current_job_id"] = result.job_id
            self.manager.state["current_document_number"] = result.document_number
