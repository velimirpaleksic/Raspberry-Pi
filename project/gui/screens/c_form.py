...
class FormScreen(tk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        combo_list_font = ("Arial", max(18, int(round(18 * self.ui_scale))))
    ...
    def _make_combo_field(self, parent, label: str, variable: tk.StringVar, values, label_font, *, popup_rows: int | None = None) -> ttk.Combobox:
        ...
        combo.bind("<FocusIn>", self._clear_keyboard_active, add=True)
        combo.bind("<ButtonPress-1>", self._clear_keyboard_active, add=True)
        combo.bind("<ButtonRelease-1>", lambda e, c=combo: self.after(25, lambda: self._enlarge_combo_popup(c)), add=True)
        combo.bind("<<ComboboxSelected>>", lambda e: self._focus_next_required(), add=True)
        return combo
    ...
    def _bind_entries_to_keyboard(self) -> None:
        if not self.kbd:
            return
        self.kbd.bind_entry(self.ime_entry, mode="alpha", uppercase_first=True, one_word=False, max_words=2, on_change=self._on_virtual_keyboard_change)
    ...
    def _normalize_two_words(self, text: str) -> str:
        raw = self._clean_two_word_raw(text).strip()
        parts = [p for p in raw.split() if p][:2]
        return " ".join(part[:1].upper() + part[1:].lower() for part in parts)

    def _split_student_name(self, text: str) -> tuple[str, str]:
        normalized = self._normalize_two_words(text)
    ...
    def _clean_two_word_raw(self, text: str) -> str:
        result: list[str] = []
    ...
    def _sanitize_student_name_now(self, entry: tk.Entry) -> None:
        if self._suppress_field_events or getattr(entry, "field_key", "") != self.STUDENT_NAME_FIELD:
            return
        current = entry.get()
        cleaned = self._clean_two_word_raw(current)
        if current == cleaned:
        ...
        try:
            ...
            cursor = len(cleaned)
        cleaned_before_cursor = self._clean_two_word_raw(current[:cursor])
    ...
    def _student_name_focus_out(self, event=None):
        entry = getattr(event, "widget", None)
        if isinstance(entry, tk.Entry):
            normalized = self._normalize_two_words(entry.get())
    ...
    def _apply_form_data(self, data: dict) -> None:
        ...
        self._set_entry_text(self.ime_entry, self._normalize_two_words(f"{ime_ucenika} {prezime}".strip()))
    ...
    def fill_debug_data(self) -> None:
        try:
            ...
            self._set_entry_text(self.ime_entry, self._normalize_two_words(combined_name))
    ...
    def _submit_impl(self):
        ...
        if not ime_ucenika or not prezime:
            missing.append("Име и презиме ученика (тачно 2 ријечи)")
