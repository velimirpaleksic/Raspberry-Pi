...
from project.gui.ui_components import TouchListPicker, add_placeholder
...
class FormScreen(tk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        self.razlog_cb = self._make_touch_picker_field(card, "Разлог", self.razlog_var, config.RAZLOZI, label_font)
    ...
    def _make_touch_picker_field(self, parent, label: str, variable: tk.StringVar, values, label_font) -> TouchListPicker:
        frame = tk.Frame(parent, bg="white")
        frame.grid_columnconfigure(0, weight=1)
        tk.Label(frame, text=label, bg="white", fg="#111111", font=label_font).grid(row=0, column=0, sticky="w")
        picker = TouchListPicker(
            frame,
            textvariable=variable,
            values=values,
            overlay_parent=self.manager or self.winfo_toplevel(),
            page_size=5,
            on_selected=self._on_reason_selected,
            placeholder="Изаберите разлог",
            ui_scale=self.ui_scale,
            font=("Arial", max(17, int(round(17 * self.ui_scale))), "bold"),
            padx=10,
            pady=max(8, int(round(8 * self.ui_scale))),
        )
        picker.grid(row=1, column=0, sticky="ew", pady=(3, 0))
        return picker

    def _on_reason_selected(self, _value: str) -> None:
    ...
    def _bind_field_validation(self) -> None:
        ...
        self.mjesto_entry.bind("<KeyRelease>", self._schedule_municipality_autofill, add=True)
        self.mjesto_entry.bind("<<VKChanged>>", self._schedule_municipality_autofill, add=True)
        self.mjesto_entry.bind("<FocusOut>", self._municipality_autofill_from_event, add=True)
        self.opstina_entry.bind("<KeyRelease>", self._track_manual_municipality, add=True)
        self.opstina_entry.bind("<<VKChanged>>", self._track_manual_municipality, add=True)
    ...
    def _focus_widget(self, widget) -> None:
        if isinstance(widget, TouchListPicker):
    ...
    def _on_virtual_keyboard_change(self, entry: tk.Entry) -> None:
        try:
            ...
            if field_key == self.STUDENT_NAME_FIELD:
                ...
                self._on_day_input()
            elif field_key == "mjesto":
                self._apply_mapped_municipality()
            elif field_key == "opstina":
                self._track_manual_municipality()
            self._clear_validation()
        except Exception:
    ...
    def _schedule_municipality_autofill(self, event=None) -> None:
        try:
            self.after_idle(self._apply_mapped_municipality)
        except Exception:
            self._apply_mapped_municipality()

    def _municipality_autofill_from_event(self, event=None) -> None:
        self._apply_mapped_municipality()

    def _track_manual_municipality(self, event=None) -> None:
        if self._suppress_field_events or not self._auto_opstina_value:
            return
        current = self._normalize_title(self.opstina_entry.get())
        if current != self._auto_opstina_value:
            self._auto_opstina_value = ""

    def _apply_mapped_municipality(self, *, force: bool = False) -> str:
        if self._suppress_field_events:
            return ""

        place_raw = str(self.mjesto_entry.get() or "").strip()
        place = "" if place_raw == config.MJESTO_PLACEHOLDER else self._normalize_title(place_raw)
        mapped = municipality_for_place(place)
        current_raw = str(self.opstina_entry.get() or "").strip()
        current = "" if current_raw == config.OPSTINA_PLACEHOLDER else self._normalize_title(current_raw)

        if mapped:
            if force or not current or current == self._auto_opstina_value:
                self._set_entry_text(self.opstina_entry, mapped, placeholder=config.OPSTINA_PLACEHOLDER)
                self._auto_opstina_value = mapped
            return mapped

        # If the user changes an automatically mapped place to an unknown one,
        # remove only our old automatic value. Never overwrite a manual value.
        if self._auto_opstina_value and current == self._auto_opstina_value:
    ...
    def _is_empty_widget(self, widget) -> bool:
        ...
        if widget is self.mjesto_entry:
            ...
            return not raw or raw == config.MJESTO_PLACEHOLDER
        if widget is self.opstina_entry:
            raw = str(widget.get() or "").strip()
            return not raw or raw == config.OPSTINA_PLACEHOLDER
    ...
    def _clear_validation(self) -> None:
        ...
        for _label, target in getattr(self, "_required_order", []):
            ...
            elif isinstance(target, TouchListPicker):
                target.set_invalid(False)
    ...
    def _mark_invalid(self, widgets) -> None:
        for widget in widgets:
            ...
            elif isinstance(widget, TouchListPicker):
    ...
    def _apply_form_data(self, data: dict) -> None:
        ...
        self._auto_opstina_value = ""
        self._apply_mapped_municipality(force=True)
    ...
    def _submit_impl(self):
        ...
        mjesto_raw = str(self.mjesto_entry.get() or "").strip()
        opstina_raw = str(self.opstina_entry.get() or "").strip()
        mjesto = "" if mjesto_raw == config.MJESTO_PLACEHOLDER else self._normalize_title(mjesto_raw)
        opstina = "" if opstina_raw == config.OPSTINA_PLACEHOLDER else self._normalize_title(opstina_raw)
        ...
        fit_checks = (
            ("{{IME}}", ime, "Име и презиме ученика", self.ime_entry),
            ("{{RODITELJ}}", roditelj, "Име родитеља", self.roditelj_entry),
            ("{{MJESTO}}", mjesto, "Мјесто рођења", self.mjesto_entry),
            ("{{OPSTINA}}", opstina, "Општина рођења", self.opstina_entry),
            ("{{STRUKA}}", struka, "Струка", self.struka_cb),
            ("{{RAZLOG}}", razlog, "Разлог", self.razlog_cb),
        )
        for placeholder, value, field_label, widget in fit_checks:
            if value_fits_placeholder(placeholder, value):
                continue
            self._mark_invalid([widget])
            self.validation_label.config(text=f"Поље „{field_label}“ је предуго за исправан изглед потврде.")
            self._focus_widget(widget)
            return None
