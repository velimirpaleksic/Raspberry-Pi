...
class DoneScreen(tk.Frame):
    def __init__(self, parent, manager=None):
        ...
        self.document_number_label = tk.Label(self, text="", font=("Arial", 18, "bold"))
        self.document_number_label.pack(pady=(8, 4))
    ...
    def on_show(self):
        doc_no = self.manager.state.get("current_document_number") if self.manager else None
        if doc_no:
            self.document_number_label.config(text=f"Дјеловодни број: {doc_no}")
        else:
            self.document_number_label.config(text="")
