...
class ScreenManager(tk.Tk):
    ...
    def post_ui_action(self, callback) -> None:
        """Queue a callback for the Tk main thread.

        Worker threads must use this method instead of calling ``after`` or
        querying widgets directly. Tk calls from background threads are not
        reliable on the Raspberry Pi kiosk runtime.
        """
