...
class TelegramControlBot:
    ...
    def _run_rollback(self, repo_root: Path, target_ref: str, current_commit: str) -> tuple[bool, str, str]:
        ...
        update_script = repo_root / "update_uvjerenja_terminal.sh"
        if not update_script.is_file():
            return False, "\n\n".join(outputs + [f"Safe update script was not found: {update_script}"]), ""

        # Use the hardened updater for rollback as well. It owns the source
        # reset, installed/source env backups, installer run and verification.
        rollback_command = [
            "env",
            f"POTVRDE_UPDATE_TARGET={resolved_commit}",
            "bash",
            str(update_script),
        ]
        ok, output = self._run_process(
            rollback_command,
            cwd=repo_root,
            timeout=config.TELEGRAM_COMMAND_TIMEOUT,
        )
        outputs.append(f"$ {self._format_command(rollback_command)}\n{output}".strip())
        if not ok:
            return False, "\n\n".join(outputs), ""
    ...
    def _git_repository_root(self) -> Path | None:
        return (
            self._find_git_root(config.APP_ROOT)
            or self._find_git_root(config.UPDATE_SOURCE_DIR)
            or self._find_git_root(Path.cwd())
        )
