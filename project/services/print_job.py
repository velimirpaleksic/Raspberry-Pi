...
def _fail(job_dir: Path, payload: Dict, job_id: str, error_code: str, user_message: str, detail: str = "", *, docx_path: str | None = None, pdf_path: str | None = None) -> PrintResult:
    ...
    try:
        ...
        log_error(f"[JOB] {job_id or '<unknown>'} could not persist failure state: {exc}")
    try:
        _notify_job_failure(job_id, payload, error_code, user_message, detail, docx_path=docx_path, pdf_path=pdf_path)
    except Exception as exc:
        log_error(f"[JOB] {job_id or '<unknown>'} could not queue failure notification: {exc}")
    try:
        check_storage_pressure_async(reason=f"print-failed:{error_code}")
    except Exception as exc:
        log_error(f"[JOB] {job_id or '<unknown>'} could not queue storage check: {exc}")
...
def _validate_form_data(form_data: Dict[str, str]) -> tuple[bool, str]:
    ...
    fit_checks = {
        ...
        "{{RODITELJ}}": form_data.get("roditelj", ""),
        "{{MJESTO}}": form_data.get("mjesto", ""),
        "{{OPSTINA}}": form_data.get("opstina", ""),
        "{{STRUKA}}": form_data.get("struka", ""),
        "{{RAZLOG}}": form_data.get("razlog", ""),
    }
    if any(not value_fits_placeholder(key, value) for key, value in fit_checks.items()):
        return False, "Jedan od podataka je predugačak za ispravan izgled potvrde. Vrati se na unos i skrati označeno polje."
...
def _normalize_form_data(form_data: Dict) -> Dict[str, str]:
    ...
    mapped_municipality = municipality_for_place(data.get("mjesto", ""))
    if mapped_municipality:
        data["opstina"] = mapped_municipality
    return data
...
def _run_print_job_impl(
    ...
    *,
    ...
    def status(code: str) -> None:
        if on_status:
            try:
                ...
                log_error(f"[JOB] Status callback failed for {code}: {exc}")
    ...
    try:
        ...
        printed = False
        if do_print:
            # Build local artifacts first. Network/Wi-Fi/printer availability is
            # deliberately checked only after DOCX and PDF generation so an
            # external dependency cannot block the local part of the job.
            ...
            if printer_message and printer_ready:
                ...
                try:
                    ...
                    log_error(f"[JOB] {job_id} could not queue printer fallback notification: {exc}")
        ...
        # Once CUPS accepted the job, bookkeeping failures must not present a
        # false print error: a retry could produce a duplicate certificate.
        try:
            _write_job_json(job_dir, payload)
        except Exception as exc:
            log_error(f"[JOB] {job_id} could not persist completion state: {exc}")
        try:
            cleanup_metadata = cleanup_print_job_documents(job_dir, output_docx, pdf_path)
            payload.update(cleanup_metadata)
        except Exception as exc:
            log_error(f"[JOB] {job_id} post-print document cleanup failed: {exc}")
        try:
            _write_job_json(job_dir, payload)
        except Exception as exc:
            log_error(f"[JOB] {job_id} could not persist cleanup state: {exc}")
        try:
            _notify_job_success(job_id, payload)
        except Exception as exc:
            log_error(f"[JOB] {job_id} could not queue success notification: {exc}")
        try:
            check_storage_pressure_async(reason="print-success")
        except Exception as exc:
            log_error(f"[JOB] {job_id} could not queue storage check: {exc}")
...
def run_print_job(
    ...
    *,
    ...
    """Run a print job and always return a controlled result.

    The implementation has detailed stage-specific error handling, while this
    outer guard also covers failures that happen before a job directory or the
    normal stage handlers are available. A worker thread must never disappear
    without giving the kiosk UI a terminal state.
    """

    try:
        return _run_print_job_impl(form_data, on_status=on_status, do_print=do_print)
    except Exception as exc:
        log_error(f"[JOB] Unhandled print pipeline error: {exc}")
        return PrintResult(
            False,
            "",
            error_code="PRINT_PIPELINE_EXCEPTION",
            user_message="Дошло је до грешке током обраде. Молимо вас, покушајте поново.",
            detail=repr(exc),
        )
