...
CLEANUP_SERVICE_FILE="/etc/systemd/system/${APP_ID}-cleanup.service"
CLEANUP_TIMER_FILE="/etc/systemd/system/${APP_ID}-cleanup.timer"
FIRSTBOOT_SERVICE_FILE="/etc/systemd/system/${APP_ID}-firstboot-finalize.service"
...
if systemctl list-unit-files 2>/dev/null | grep -q "^NetworkManager.service"; then
  log_info "Best-effort enabling NetworkManager for touchscreen Wi‑Fi setup..."
...
# Djelovodni broj seed (first real document will increment from here)
POTVRDE_DJELOVODNI_BROJ="$DEFAULT_DJELOVODNI_BROJ"
POTVRDE_YEAR_MODE="auto"
POTVRDE_MANUAL_YEAR="$(date +%Y)"
POTVRDE_ADMIN_PIN="1234"
...
After=graphical.target network-online.target cups.service
...
Environment=APP_ROOT=$SRC_DST
Environment=PYTHON_BIN=$VENV_DIR/bin/python
ExecStart=/usr/bin/env bash $SRC_DST/scripts/launch_terminal.sh
Restart=always
RestartSec=3
TimeoutStartSec=90
TimeoutStopSec=20
...
log_info "Installing cleanup service + timer"
sudo tee "$CLEANUP_SERVICE_FILE" >/dev/null <<EOF
[Unit]
Description=$APP_TITLE daily cleanup
...
Environment=APP_ROOT=$SRC_DST
Environment=PYTHON_BIN=$VENV_DIR/bin/python
ExecStart=/usr/bin/env bash $SRC_DST/scripts/maintenance_daily.sh
EOF

sudo tee "$CLEANUP_TIMER_FILE" >/dev/null <<EOF
[Unit]
Description=$APP_TITLE daily cleanup timer

[Timer]
OnCalendar=daily
Persistent=true
RandomizedDelaySec=5m
Unit=$(basename "$CLEANUP_SERVICE_FILE")

[Install]
WantedBy=timers.target
EOF

log_info "Installing first boot finalize service"
sudo tee "$FIRSTBOOT_SERVICE_FILE" >/dev/null <<EOF
[Unit]
Description=$APP_TITLE first boot finalize
After=graphical.target network-online.target cups.service
Wants=network-online.target cups.service
ConditionPathExists=!$VAR_DIR/firstboot/complete.flag
...
Environment=APP_ROOT=$SRC_DST
Environment=PYTHON_BIN=$VENV_DIR/bin/python
ExecStart=/usr/bin/env bash $SRC_DST/scripts/first_boot_finalize.sh
RemainAfterExit=yes
...
sudo systemctl enable --now "$(basename "$CLEANUP_TIMER_FILE")"
sudo systemctl enable "$(basename "$FIRSTBOOT_SERVICE_FILE")" || true
log_ok "Service enabled + started: $APP_ID.service"
log_ok "Cleanup timer enabled: $(basename "$CLEANUP_TIMER_FILE")"
log_ok "First boot finalize installed: $(basename "$FIRSTBOOT_SERVICE_FILE")"
...
log_info "Cleanup timer: systemctl status $(basename "$CLEANUP_TIMER_FILE")"
