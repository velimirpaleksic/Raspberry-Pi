...
- prije izmjena pravi privatnu sigurnosnu kopiju instaliranog env fajla u `/var/lib/uvjerenja-terminal/env-backups`
- privremeno sklanja source `.env` izvan Git work tree-ja i vraca ga i nakon neuspjelog update-a
- lokalni source resetuje na `origin/<branch>` da lokalne izmjene ne blokiraju update
- installer smije dodati novu env postavku, ali ne smije promijeniti ili ukloniti nijednu postojecu vrijednost
- ako se postojeca env vrijednost promijeni, originalni env se automatski vraca i update se oznacava neuspjesnim
...
Telegram `/rollback [git-ref]` koristi isti zasticeni updater sa provjerenim commitom,
ponovo pokrece installer za stari commit i tek zatim otvara vracenu verziju aplikacije.
